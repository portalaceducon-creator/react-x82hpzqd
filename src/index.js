...
const SKILL_SUBS = {
  speaking: ["Fluencia","Pronuncia","Vocabulario","Gramatica"],
  reading:  ["Compreensao","Velocidade","Vocabulario","Inferencia"],
  listening:["Compreensao","Sotaques","Anotacoes"],
  writing:  ["Estrutura","Gramatica","Vocabulario","Coerencia"],
};

const MONTHS = ["Jan","Fev","Mar","Abr","Mai","Jun","Jul","Ago","Set","Out","Nov","Dez"];
const MFULL  = ["Janeiro","Fevereiro","Marco","Abril","Maio","Junho","Julho","Agosto","Setembro","Outubro","Novembro","Dezembro"];
const WDAYS  = ["Dom","Seg","Ter","Qua","Qui","Sex","Sab"];
const WFULL  = ["Domingo","Segunda-feira","Terca-feira","Quarta-feira","Quinta-feira","Sexta-feira","Sabado"];
...
const LESSON_STATUS = {
  ...
  no_show:           { label:"Nao apareceu",  color:"#ff4d4d", bg:"rgba(255,77,77,0.15)"  },
};
const TASK_STATUS = {
  done:     { label:"Feitas",       color:"#c8f135" },
  partial:  { label:"Parcialmente", color:"#f0a500" },
  not_done: { label:"Nao fez",      color:"#ff4d4d" },
  none:     { label:"Sem tarefa",   color:"#555" },
};
...
const BG='#111111', CARD='#1e1e1e', CARD2='#272727', BORDER='#333333';
const TEXT='#ffffff', TEXT2='#aaaaaa', TEXT3='#666666';
const LIME='#c8f135', BLUE='#4da6ff', CORAL='#ff6b6b', AMBER='#f0a500', PURPLE='#a78bfa';
    ...
    <div style={{minHeight:'100vh',background:BG,display:'flex',alignItems:'center',justifyContent:'center',padding:24}}>
      ...
      <div style={{width:'100%',maxWidth:400}}>
        <div style={{textAlign:'center',marginBottom:40}}>
          ...
          <div style={{color:TEXT3,fontSize:14,marginTop:6}}>Ingles Profissional - Curitiba, PR</div>
        </div>
        <div style={{background:CARD,borderRadius:24,padding:28,border:`1px solid ${BORDER}`}}>
          <div style={{display:'flex',background:BG,borderRadius:14,padding:4,marginBottom:24,gap:4}}>
            {[['login','Entrar'],['register','Cadastrar']].map(([k,l])=>(
              <button key={k} onClick={()=>setMode(k)} className="tap"
                style={{flex:1,padding:'12px',borderRadius:11,border:'none',cursor:'pointer',fontWeight:700,fontSize:15,background:mode===k?LIME:'transparent',color:mode===k?'#111':TEXT3}}>
                {l}
              </button>
            ))}
          </div>
          ...
          <button onClick={submit} disabled={loading} className="tap"
            style={{width:'100%',padding:18,borderRadius:14,border:'none',background:LIME,color:'#111',fontSize:16,fontWeight:800,cursor:'pointer'}}>
...
function OverviewTab({ studentId }) {
  ...
  const stats=[
    ...
    {label:'Presenca',value:rate+'%',color:rate>=80?LIME:rate>=60?AMBER:CORAL},
    {label:'Gramatica',value:achieved+'/84',color:AMBER},
    {label:'Tarefas',value:tasksDone,color:PURPLE},
    {label:'Progresso',value:inProg,color:CORAL},
  ];
  return(
    <div className="fade" style={{padding:20,paddingBottom:100}}>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:20}}>
        {stats.map(s=>(
          <div key={s.label} style={{background:CARD,borderRadius:20,padding:20,border:`1px solid ${BORDER}`}}>
            ...
            <div style={{fontSize:13,fontWeight:600,color:TEXT3,marginTop:6,textTransform:'uppercase',letterSpacing:'0.06em'}}>{s.label}</div>
          </div>
        ))}
      </div>
      <div style={{background:CARD,borderRadius:20,padding:20,border:`1px solid ${BORDER}`,marginBottom:16}}>
        <div style={{fontSize:13,fontWeight:700,color:TEXT3,textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:16}}>Aulas por Mes</div>
      ...
      <div style={{background:CARD,borderRadius:20,padding:20,border:`1px solid ${BORDER}`}}>
        <div style={{fontSize:13,fontWeight:700,color:TEXT3,textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:4}}>Gramatica</div>
        {[{n:'Atingido',v:achieved,c:LIME},{n:'Em progresso',v:inProg,c:AMBER},{n:'Nao atingido',v:grammar.filter(g=>g.status==='not_achieved').length,c:CORAL}].map(g=>(
...
function CalendarTab({ studentId, isTeacher }) {
  ...
  const saveLesson=async(date,data)=>{
    const ex=lessons[date];
    if(ex) await supabase.from('lessons').update(data).eq('id',ex.id);
    ...
    <div onClick={e=>e.target===e.currentTarget&&onClose()} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',display:'flex',alignItems:'flex-end',justifyContent:'center',zIndex:1000,backdropFilter:'blur(8px)'}}>
      <div style={{background:CARD,borderRadius:'24px 24px 0 0',padding:24,width:'100%',maxWidth:540,maxHeight:'90vh',overflowY:'auto',border:`1px solid ${BORDER}`}}>
        ...
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
          ...
          <button onClick={onClose} style={{background:CARD2,border:'none',borderRadius:10,width:36,height:36,fontSize:20,cursor:'pointer',color:TEXT3}}>x</button>
        </div>
        <div style={{marginBottom:18}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
            {Object.entries(LESSON_STATUS).map(([k,c])=>(
              <div key={k} className="tap" onClick={()=>setStatus(k)}
                style={{background:status===k?c.bg:CARD2,border:`2px solid ${status===k?c.color:BORDER}`,borderRadius:14,padding:'12px 10px',display:'flex',alignItems:'center',gap:8,cursor:'pointer'}}>
        ...
        <div style={{marginBottom:18}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
            {Object.entries(TASK_STATUS).map(([k,c])=>(
              <div key={k} className="tap" onClick={()=>setTasks(k)}
                style={{background:tasks===k?`${c.color}22`:CARD2,border:`2px solid ${tasks===k?c.color:BORDER}`,borderRadius:14,padding:'12px',cursor:'pointer',textAlign:'center'}}>
        ...
        <div style={{marginBottom:20}}>
          <div style={{fontSize:12,fontWeight:700,color:TEXT3,textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:10}}>Observacoes</div>
          <textarea value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Notas..."
            style={{width:'100%',background:CARD2,border:`1px solid ${BORDER}`,borderRadius:14,padding:'14px',fontSize:15,color:TEXT,resize:'vertical',minHeight:80}}/>
        </div>
...
function GrammarTab({ studentId, isTeacher }) {
  ...
  const sConfig={achieved:{label:'Atingido',color:LIME},in_progress:{label:'Em progresso',color:AMBER},not_achieved:{label:'Nao atingido',color:CORAL}};
    ...
    <div className="fade" style={{paddingBottom:100}}>
      <div style={{background:CARD,borderBottom:`1px solid ${BORDER}`,padding:'16px 20px',position:'sticky',top:0,zIndex:10}}>
        <div style={{display:'flex',background:BG,borderRadius:14,padding:4,gap:4,marginBottom:sub==='grammar'?12:0}}>
          {[['grammar','Gramatica'],['skills','Habilidades']].map(([k,l])=>(
            <button key={k} onClick={()=>setSub(k)} className="tap"
              style={{flex:1,padding:'12px',borderRadius:11,fontSize:15,fontWeight:700,border:'none',cursor:'pointer',background:sub===k?LIME:'transparent',color:sub===k?'#111':TEXT3}}>
              {l}
            </button>
          ))}
        </div>
        {sub==='grammar'&&<input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar topico..."
          style={{width:'100%',background:CARD2,border:`1px solid ${BORDER}`,borderRadius:14,padding:'14px 16px',fontSize:15,color:TEXT}}/>}
      </div>
      {sub==='grammar'&&(
                      ...
                      <div style={{display:'flex',gap:6}}>
                        {Object.entries(sConfig).map(([k,c])=>(
                          <button key={k} onClick={()=>setGStatus(t.id,st===k?'':k)} disabled={!isTeacher} className="tap"
                            style={{flex:1,border:`2px solid ${st===k?c.color:BORDER}`,borderRadius:10,padding:'9px 4px',cursor:isTeacher?'pointer':'default',
                              background:st===k?`${c.color}20`:CARD2,fontSize:12,fontWeight:st===k?700:500,color:st===k?c.color:TEXT3,textAlign:'center'}}>
                            {c.label}
                          </button>
                        ))}
                      </div>
              ...
              {items.map((item,idx)=>{
                ...
                return(
                  <div key={item} style={{padding:'14px 20px',borderBottom:'1px solid #222'}}>
                    ...
                    <div style={{display:'flex',gap:6}}>
                      {[1,2,3,4,5].map(n=>(
                        <button key={n} onClick={()=>setSkillVal(sk,idx,n)} disabled={!isTeacher} className="tap"
                          style={{flex:1,height:34,borderRadius:8,border:'none',cursor:isTeacher?'pointer':'default',background:n<=val?skillColors[sk]:CARD2,transition:'background 0.15s'}}/>
                      ))}
                    </div>
                  </div>
                );
    ...
    <div className="fade" style={{padding:20,paddingBottom:100}}>
      ...
      {isTeacher&&adding&&(
        <div style={{background:CARD,borderRadius:20,padding:20,marginBottom:16,border:`1px solid ${BORDER}`}}>
          <div style={{fontSize:13,fontWeight:700,color:TEXT3,textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:14}}>Nova Tarefa</div>
          <input value={title} onChange={e=>setTitle(e.target.value)} placeholder="Titulo *" style={ipt}/>
          <textarea value={desc} onChange={e=>setDesc(e.target.value)} placeholder="Descricao" style={{...ipt,resize:'vertical',minHeight:70}}/>
      ...
      {pending.length>0&&(
        <div style={{background:CARD,borderRadius:20,overflow:'hidden',border:`1px solid ${BORDER}`,marginBottom:14}}>
          <div style={{padding:'14px 20px',borderBottom:`1px solid ${BORDER}`}}>
            <span style={{fontSize:16,fontWeight:800,color:TEXT}}>Pendentes <span style={{color:LIME}}>{pending.length}</span></span>
          </div>
          {pending.map((item,i)=>(
            <div key={item.id} style={{padding:'16px 20px',borderBottom:i<pending.length-1?`1px solid ${BORDER}`:'none',display:'flex',alignItems:'flex-start',gap:14}}>
              ...
              <div style={{flex:1}}>
                ...
                {item.due_date&&<div style={{fontSize:13,color:LIME,fontWeight:600}}>Ate {new Date(item.due_date+'T12:00').toLocaleDateString('pt-BR')}</div>}
              </div>
              {isTeacher&&<button onClick={()=>remove(item.id)} className="tap" style={{background:CARD2,border:'none',borderRadius:8,padding:'6px 10px',color:TEXT3,cursor:'pointer',fontSize:16}}>x</button>}
            </div>
          ))}
        </div>
      )}
      {done.length>0&&(
        <div style={{background:CARD,borderRadius:20,overflow:'hidden',border:`1px solid ${BORDER}`,opacity:0.6}}>
          <div style={{padding:'14px 20px',borderBottom:`1px solid ${BORDER}`}}>
            <span style={{fontSize:16,fontWeight:800,color:TEXT3}}>Concluidas {done.length}</span>
          </div>
          {done.map((item,i)=>(
            <div key={item.id} style={{padding:'14px 20px',borderBottom:i<done.length-1?`1px solid ${BORDER}`:'none',display:'flex',alignItems:'center',gap:14}}>
              <button onClick={()=>toggle(item.id,item.done)} className="tap" style={{width:24,height:24,borderRadius:8,border:'none',background:LIME,cursor:'pointer',flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',color:'#111',fontSize:12,fontWeight:900}}>v</button>
...
function MaterialsTab({ studentId, isTeacher }) {
  ...
  const ipt={width:'100%',background:BG,border:`1px solid ${BORDER}`,borderRadius:14,padding:'14px 16px',fontSize:15,color:TEXT,marginBottom:10};
  const typeColors={link:BLUE,pdf:CORAL,audio:AMBER,exercise:LIME,book:PURPLE};
    ...
    <div className="fade" style={{padding:20,paddingBottom:100}}>
      ...
      {isTeacher&&adding&&(
        <div style={{background:CARD,borderRadius:20,padding:20,marginBottom:16,border:`1px solid ${BORDER}`}}>
          ...
          <select value={type} onChange={e=>setType(e.target.value)} style={{...ipt,background:BG}}>
            <option value="link">Link</option><option value="pdf">PDF</option><option value="audio">Audio</option><option value="exercise">Exercicio</option><option value="book">Livro</option>
          </select>
          <input value={url} onChange={e=>setUrl(e.target.value)} placeholder="URL" style={ipt}/>
          <textarea value={desc} onChange={e=>setDesc(e.target.value)} placeholder="Descricao" style={{...ipt,resize:'vertical',minHeight:60,marginBottom:16}}/>
          <div style={{display:'flex',gap:8}}>
            <button onClick={()=>setAdding(false)} className="tap" style={{flex:1,padding:'14px',borderRadius:14,border:`1px solid ${BORDER}`,background:'transparent',color:TEXT3,cursor:'pointer',fontSize:14,fontWeight:600}}>Cancelar</button>
            <button onClick={add} className="tap" style={{flex:2,padding:'14px',borderRadius:14,border:'none',background:LIME,color:'#111',cursor:'pointer',fontSize:15,fontWeight:800}}>Salvar</button>
          </div>
      ...
      {items.map(item=>(
        <div key={item.id} className="tap" style={{background:CARD,borderRadius:18,marginBottom:10,padding:'18px',display:'flex',alignItems:'center',gap:16,border:`1px solid ${BORDER}`}}>
          <div style={{width:52,height:52,borderRadius:14,background:`${typeColors[item.type]||BLUE}22`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:24,flexShrink:0}}>{typeIcons[item.type]||'📎'}</div>
          ...
          {isTeacher&&<button onClick={()=>remove(item.id)} className="tap" style={{background:`${CORAL}22`,border:`1px solid ${CORAL}`,borderRadius:10,padding:'8px 12px',color:CORAL,cursor:'pointer',fontSize:14,fontWeight:700}}>x</button>}
        </div>
      ))}
    </div>
  );
...
function VideosTab({ studentId, isTeacher }) {
  ...
  return(
    <div className="fade" style={{padding:20,paddingBottom:100}}>
      {isTeacher&&!adding&&<button onClick={()=>setAdding(true)} className="tap" style={{width:'100%',padding:18,borderRadius:16,border:`2px dashed ${CORAL}`,background:'transparent',color:CORAL,fontSize:16,fontWeight:700,cursor:'pointer',marginBottom:16}}>+ Adicionar Video</button>}
      {isTeacher&&adding&&(
        <div style={{background:CARD,borderRadius:20,padding:20,marginBottom:16,border:`1px solid ${BORDER}`}}>
          <input value={title} onChange={e=>setTitle(e.target.value)} placeholder="Titulo *" style={ipt}/>
      ...
      {items.length===0&&<div style={{textAlign:'center',padding:'60px 0',color:TEXT3}}><div style={{fontSize:48,marginBottom:12}}>🎬</div><div style={{fontSize:16,fontWeight:600}}>Nenhum video</div></div>}
      {items.map(item=>{
        ...
        return(
          <div key={item.id} style={{background:CARD,borderRadius:20,marginBottom:16,overflow:'hidden',border:`1px solid ${BORDER}`}}>
            {ytId?(<div style={{position:'relative',paddingBottom:'56.25%',background:'#000'}}><iframe src={`https://www.youtube.com/embed/${ytId}`} title={item.title} style={{position:'absolute',top:0,left:0,width:'100%',height:'100%',border:'none'}} allowFullScreen/></div>):(
              <div className="tap" onClick={()=>window.open(item.url,'_blank')} style={{background:CARD2,padding:'28px',textAlign:'center',cursor:'pointer'}}><div style={{fontSize:36,color:CORAL}}>play</div></div>
            )}
...
function ScheduleTab({ studentId, isTeacher }) {
  ...
  return(
    <div className="fade" style={{padding:20,paddingBottom:100}}>
      <div style={{background:CARD,borderRadius:20,padding:20,border:`1px solid ${BORDER}`}}>
        <div style={{fontSize:13,fontWeight:700,color:TEXT3,textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:16}}>Horarios Fixos</div>
        {slots.length===0&&<div style={{textAlign:'center',padding:'24px 0',color:TEXT3,fontSize:15}}>Nenhum horario configurado</div>}
        {slots.map((sl,i)=>(
          <div key={i} style={{background:CARD2,borderRadius:16,padding:16,marginBottom:12}}>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,marginBottom:12}}>
              <div><div style={{fontSize:11,fontWeight:700,color:TEXT3,textTransform:'uppercase',marginBottom:6}}>Dia</div>
              <select value={sl.day_of_week??1} onChange={e=>upd(i,'day_of_week',Number(e.target.value))} disabled={!isTeacher} style={sel}>{WFULL.map((d,idx)=><option key={idx} value={idx}>{d.slice(0,3)}</option>)}</select></div>
              <div><div style={{fontSize:11,fontWeight:700,color:TEXT3,textTransform:'uppercase',marginBottom:6}}>Inicio</div>
              <input type="time" value={sl.start_time??'10:00'} onChange={e=>upd(i,'start_time',e.target.value)} disabled={!isTeacher} style={sel}/></div>
              <div><div style={{fontSize:11,fontWeight:700,color:TEXT3,textTransform:'uppercase',marginBottom:6}}>Duracao</div>
              <select value={sl.duration??60} onChange={e=>upd(i,'duration',Number(e.target.value))} disabled={!isTeacher} style={sel}>{[30,45,60,75,90,120].map(d=><option key={d} value={d}>{d}min</option>)}</select></div>
            </div>
            <div style={{background:`${LIME}15`,borderRadius:10,padding:'10px 14px',fontSize:14,color:LIME,fontWeight:600,display:'flex',justifyContent:'space-between'}}>
              <span>{WFULL[sl.day_of_week??1]} - {sl.start_time??'10:00'} ate {endTime(sl.start_time??'10:00',sl.duration??60)}</span>
  ...
  return(
    <div className="fade" style={{padding:20,paddingBottom:100}}>
      ...
      <div style={{background:CARD,borderRadius:20,padding:20,border:`1px solid ${BORDER}`}}>
        ...
        <div><div style={{fontSize:12,fontWeight:700,color:TEXT3,textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:6}}>Data de inicio</div>
        <input type="date" value={f.start_date||''} onChange={e=>upd('start_date',e.target.value)} disabled={!isTeacher} style={ipt(!isTeacher)}/></div>
        <div><div style={{fontSize:12,fontWeight:700,color:TEXT3,textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:6}}>Nivel</div>
        ...
        <input value={f.goal||''} onChange={e=>upd('goal',e.target.value)} placeholder="Ex: Negocios, Viagens..." disabled={!isTeacher} style={ipt(!isTeacher)}/></div>
        <div><div style={{fontSize:12,fontWeight:700,color:TEXT3,textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:6}}>Observacoes</div>
        ...
        <div style={{padding:'8px 20px 32px'}}>
          ...
          {students.map(s=>(
            <div key={s.id} className="tap" onClick={()=>{onSelect(s.id);onClose();}}
              ...
              {activeId===s.id&&<div style={{color:LIME,fontSize:18,fontWeight:900}}>v</div>}
            </div>
          ))}
...
const TABS=[
  ...
  {id:'calendar',label:'Aulas'},
  {id:'grammar', label:'Gramatica'},
  {id:'homework',label:'Tarefas'},
...
export default function App() {
  ...
  if(loading) return(
    <div style={{height:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'#111111'}}>
      <style>{CSS}</style>
      <div style={{color:LIME,fontSize:24,fontWeight:800}}>Carregando...</div>
    </div>
  );
  ...
  return(
    <div style={{height:'100vh',display:'flex',flexDirection:'column',background:BG}}>
      ...
      <div style={{background:CARD,padding:'14px 20px',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0,borderBottom:`1px solid ${BORDER}`}}>
        ...
        <div style={{display:'flex',gap:8}}>
          {isTeacher&&(
            <button onClick={()=>setShowStudents(true)} className="tap"
              style={{background:CARD2,border:`1px solid ${BORDER}`,borderRadius:12,padding:'9px 14px',color:TEXT,cursor:'pointer',fontSize:14,fontWeight:700,display:'flex',alignItems:'center',gap:6,maxWidth:140}}>
              <span style={{overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{activeStudent?.name||'Alunos'}</span>
              <span style={{color:TEXT3,fontSize:11}}>v</span>
            </button>
          )}
      ...
      {isTeacher&&activeStudent&&(
        <div style={{background:CARD2,padding:'10px 20px',display:'flex',alignItems:'center',gap:10,borderBottom:`1px solid ${BORDER}`}}>
          ...
          <button onClick={()=>{setActiveId(null);setShowStudents(true);}} className="tap"
            style={{background:'transparent',border:`1px solid ${BORDER}`,borderRadius:10,padding:'6px 12px',color:TEXT3,cursor:'pointer',fontSize:13,fontWeight:600}}>
            Trocar
          </button>
        </div>
      )}
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)'}}>
          {tabsRow1.map(t=>(
            <button key={t.id} onClick={()=>setTab(t.id)} className="tap"
              style={{padding:'14px 4px 12px',border:'none',background:'transparent',cursor:'pointer',display:'flex',flexDirection:'column',alignItems:'center',gap:4}}>
              <div style={{width:6,height:6,borderRadius:'50%',background:tab===t.id?LIME:'transparent'}}/>
              <span style={{fontSize:11,fontWeight:tab===t.id?800:500,color:tab===t.id?LIME:TEXT3}}>{t.label}</span>
            </button>
          ))}
        </div>
